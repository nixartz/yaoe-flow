...
- **Provider account/subscription quota exhausted** (e.g. Claude Code CLI `"You've hit your limit · resets HH:MMam (Timezone)"`): this is different from the budget caps above (which are yaoe-flow's own spend tracking) — it's the provider itself refusing the call. The scheduler detects it, pauses new dispatches on that harness (this Linear connection) until the parsed reset time (or a conservative default, `HARNESS_QUOTA_DEFAULT_COOLDOWN_MS`, when no reset time is present in the message), posts a comment on the affected issue with the provider's message and the expected resume time, and returns the issue to the queue immediately — no manual intervention needed. Enable the `harness_quota_exceeded` notification event (Dashboard → Notifications) to get alerted when this happens.
