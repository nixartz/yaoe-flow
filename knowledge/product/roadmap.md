...
- **glibc version probing in `install.sh`**: not checked today (see the 2026-08-03 avx2-baseline-builds OKF change) — a Bun binary requiring a newer glibc than an old Linux distro ships would fail with an unclear dynamic-linker error. No fallback build exists for this the way `-baseline` covers AVX2, so it's lower priority until actually reported.
