...
- ~~**`yaoe-flow update` atomic self-swap**: today it only checks the latest release and points at the idempotent installer; download + checksum + atomic rename + restart is the next step.~~ — shipped: [knowledge/changes/2026-08-11/self-update-and-install-hardening](../changes/2026-08-11/self-update-and-install-hardening/README.md).
- ~~**glibc version probing in `install.sh`**: not checked today (see the 2026-08-03 avx2-baseline-builds OKF change) — a Bun binary requiring a newer glibc than an old Linux distro ships would fail with an unclear dynamic-linker error. No fallback build exists for this the way `-baseline` covers AVX2, so it's lower priority until actually reported.~~ — shipped: [knowledge/changes/2026-08-11/self-update-and-install-hardening](../changes/2026-08-11/self-update-and-install-hardening/README.md).
